...
tabela4 <- data.frame( matrix(data=charTabela4[5:12], nrow=2, ncol=4, byrow=TRUE) );
colnames(tabela4) <- charTabela4[1:4];
```
...
tabela4 <- data.frame( matrix(data=charTabela4[4:length(charTabela4)], 
                              nrow=2, ncol=3, byrow=TRUE) );
colnames(tabela4) <- charTabela4[1:3];


charTabela4a <- grep(pattern="<header>", x=webTabela4, value=TRUE);

charTabela4a1 <- sub(pattern="<header>", replacement="", x=charTabela4a);
charTabela4a1 <- sub(pattern=" *<small.*", replacement="", x=charTabela4a1);

charTabela4a2 <- sub(pattern=".*<strong>", replacement="", x=charTabela4a);
charTabela4a2 <- sub(pattern="</strong>.*", replacement="", x=charTabela4a2);

tabela4$novo <- charTabela4a2;
colnames(tabela4)[ncol(tabela4)] <- charTabela4a1;
```
...
webTabela5 <- webPage[idsInicio[2]:(idsFim-1)];

charTabela5 <- grep(pattern="<div>|(.*?)<br>|(.*?)</div>", x=webTabela5, value=TRUE);
...
charTabela5 <- sub(pattern="<br>", replacement="", x=charTabela5);

charTabela5 <- charTabela5[ which(charTabela5!="") ];



pontosInsercao <- seq(from=6, to=length(charTabela5)+9, by=9);
for(ii in pontosInsercao){
  charTabela5 <- append(x=charTabela5, values=c(NA), after=ii );
}

pontosInsercao2 <- seq(from=14, to=(length(charTabela5)+10), by=10);
for(ii in pontosInsercao2){
  charTabela5 <- append(x=charTabela5, values=c(NA), after=ii );
}
...
tabela5 <- NULL;
pontosQuebra <- seq(from=6, to=length(charTabela5), by=10);
for(ii in pontosQuebra){
  parcialTabela5 <- matrix(data=charTabela5[ii:(ii+9)], nrow=2, ncol=5, byrow=FALSE);
  tabela5 <- rbind(tabela5, parcialTabela5);
}
tabela5 <- data.frame(tabela5);
colnames(tabela5) <- charTabela5[1:5];
```
